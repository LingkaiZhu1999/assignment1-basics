...
def train(run, args):
    ...
    for iter in range(iterations):
        model.train()
        current_lr = learning_rate_schedule(iter, args.lr_max, args.lr_min, args.iter_warmup, t_cos_anneal=args.iter_cos_annealing)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iter_warmup", type=int, default=1, help="warm up iterations")
    parser.add_argument("--lr_max", type=float, default=1e-3, help="learning rate max")
    parser.add_argument("--lr_min", type=float, default=1e-5, help="learning rate min")
